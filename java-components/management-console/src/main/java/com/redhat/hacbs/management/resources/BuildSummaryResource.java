    ...
    public BuildSummaryDTO all(@QueryParam("label") String label) {
        if (label != null && !label.isBlank()) {
            long all = (Long) entityManager.createQuery(
                    "select count(s) from StoredDependencyBuild s where s.id in (select b.id from StoredDependencyBuild  b inner join b.buildAttempts ba inner join ba.producedArtifacts a inner join MavenArtifactLabel l on l.artifact = a inner join ArtifactLabelName aln on l.name=aln where aln.name=:name)")
                    .setParameter("name", label)
                    .getSingleResult();
            long successful = (Long) entityManager.createQuery(
                    "select count(s) from StoredDependencyBuild s where s.id in (select b.id from StoredDependencyBuild  b inner join b.buildAttempts ba inner join ba.producedArtifacts a inner join MavenArtifactLabel l on l.artifact = a inner join ArtifactLabelName aln on l.name=aln where aln.name=:name and ba.successful=true) ")
                    .setParameter("name", label)
                    .getSingleResult();
            long contaminated = (Long) entityManager.createQuery(
                    "select count(s) from StoredDependencyBuild s where s.id in (select b.id from StoredDependencyBuild  b inner join b.buildAttempts ba inner join ba.producedArtifacts a inner join MavenArtifactLabel l on l.artifact = a inner join ArtifactLabelName aln on l.name=aln where aln.name=:name and ba.contaminated=true)")
                    .setParameter("name", label)
                    .getSingleResult();
