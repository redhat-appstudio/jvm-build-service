...
dev: dev-image-controller
	cd java-components && mvn clean install -Dlocal -DskipTests -Ddev && cd ../domain-proxy-go && go build -o domain_proxy_server domain_proxy_server.go common.go && go build -o domain_proxy_client domain_proxy_client.go common.go && ..
