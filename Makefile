...
dev: dev-image-controller
	cd java-components && mvn clean install -Dlocal -DskipTests -Ddev && cd ../domain-proxy-go && go build -o domain-proxy-server domainproxy_server.go common.go && go build -o domain-proxy-client domain_proxy_client.go common.go && cd ..
