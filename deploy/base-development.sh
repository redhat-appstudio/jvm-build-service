...
kubectl create --ignore-not-found secret generic jvm-build-image-secrets --from-file=.dockerconfigjson=$HOME/.docker/config.json --type=kubernetes.io/dockerconfigjson
kubectl create --ignore-not-found secret generic jvm-build-git-secrets --from-literal .git-credentials="
